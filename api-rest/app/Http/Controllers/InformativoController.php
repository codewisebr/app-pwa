...
class InformativoController extends Controller
{
    ...
    public function getinfo(){
        ...
        $infoInfo=Info::where('ativo', '1')->orderBy('updated_at', 'desc')->get();
    ...
    public function getallinfo(){
        //recebe do bd o valor 
        $infoInfo=Info::orderBy('updated_at', 'desc')->get();
    ...
    public function getnivelinfo(Request $request){
        ...
        if($request->tipo == 1){
            if($request->nivel == 1)
            {
                $Info=Info::where(['nivel'=> 1, 'ativo'=>1])->take(3)->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
            else if($request->nivel == 2){
                $Info=Info::where([['ativo',1],['nivel','<=',2]])->take(3)->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
            else if($request->nivel == 3){
                $Info=Info::where('ativo',1)->take(3)->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
        }
            ...
            if($request->nivel == 1)
            {
                $Info=Info::where(['nivel'=> 1, 'ativo'=>1])->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
            else if($request->nivel == 2){
                $Info=Info::where([['ativo',1],['nivel','<=',2]])->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
            else if($request->nivel == 3){
                $Info=Info::where('ativo',1)->orderBy('updated_at', 'desc')->get();
                return response()->json($Info);
            }
        }
    }
    ...
    public function deletehistinfo(){
        $info =  Info::where('ativo', 0)->delete();
    }
}
