...
let EditdadosPageModule = class EditdadosPageModule {
};
...
let EditdadosPage = class EditdadosPage {
    constructor(authService, alertService, navCtrl, storage, global, routingService) {
    ...
    editar(form) {
        ...
        //salva as alterações
        this.authService.updateUser(this.id, form.value.fName, form.value.lName, form.value.email, form.value.endereco, form.value.cidade, form.value.estado, form.value.data_nasc, form.value.telefone, form.value.nivel, this.global.cargo, form.value.profissao).subscribe(resp => {
            if (resp == true) {
...
//# sourceMappingURL=pages-auth-editdados-editdados-module-es2015.js.map