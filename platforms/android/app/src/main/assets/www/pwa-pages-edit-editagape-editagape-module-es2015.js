...
let EditagapePageModule = class EditagapePageModule {
};
...
let EditagapePage = class EditagapePage {
    ...
    showAgape() {
        this.authService.getAgape(0).subscribe(data => {
    ...
    editar(form) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.authService.getAgape(0).subscribe(data => {
                for (let i = 0; i < data.length; i++) {
                    if (this.id == data[i].id) {
                        this.authService.updateAgape(data[i].id, form.value.agape, 1, form.value.data).subscribe(resul => {
                            this.dismiss();
...
//# sourceMappingURL=pwa-pages-edit-editagape-editagape-module-es2015.js.map