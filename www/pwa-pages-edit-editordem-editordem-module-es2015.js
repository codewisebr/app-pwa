...
let EditordemPageModule = class EditordemPageModule {
};
...
let EditordemPage = class EditordemPage {
    ...
    showOrdem() {
        this.authService.getOrdem().subscribe(data => {
    ...
    editar(form) {
        this.authService.getOrdem().subscribe(data => {
            for (let i = 0; i < data.length; i++) {
                ...
                if (this.id == data[i].id) {
                    this.authService.updateOrdem(data[i].id, form.value.novo, 1, form.value.nivel).subscribe(data => {
                        this.dismiss();
...
//# sourceMappingURL=pwa-pages-edit-editordem-editordem-module-es2015.js.map